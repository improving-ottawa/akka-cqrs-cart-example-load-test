...
class JdbcItemPopularityRepository(
    ...
    sessionFactory: () => ScalikeJdbcSession = () => new ScalikeJdbcSession())(
    implicit val ec: ExecutionContext)
    extends ItemPopularityRepository {
  def update(itemId: String, delta: Int): Future[Unit] = {
    val session = sessionFactory()
    repoFactory(session).update(itemId, delta).andThen(_ => session.close())
  }

  def getItem(itemId: String): Future[Option[Long]] = {
    val session = sessionFactory()
    repoFactory(session).getItem(itemId).andThen(_ => session.close())
  }
}
...
class JdbcItemPopularityRepositoryFactoryImpl(
  ...
  def apply(session: ScalikeJdbcSession): ItemPopularityRepository =
    new ItemPopularityRepository {
...
object JdbcItemPopularityRepositoryFactory {
  ...
  def apply(ec: ExecutionContext): JdbcItemPopularityRepositoryFactory =
    new JdbcItemPopularityRepositoryFactoryImpl(ec)
